import { db } from '../db';
import { posts, postLikes, postAttachments, communities, communityMembers, user } from '../db/schema';
...
class PostService {
  ...
  static async createPost(authorId: string, communityId: string, data: CreatePostData) {
    ...
    if (data.postType === 'announcement') {
      ...
      if (userRole !== 'owner' && userRole !== 'moderator') {
        throw new Error('Only community owners and moderators can create announcements');
      }
    }
  ...
  static async getCommunityPosts(communityId: string, options: PostQueryOptions = {}) {
    const {
      page = 1,
      pageSize = 20,
      filters = {},
    ...
    if (filters.postType) {
      query.where(and(...baseConditions, eq(posts.postType, filters.postType as any)));
    }
    if (filters.isPinned !== undefined) {
      query.where(and(...baseConditions, eq(posts.isPinned, filters.isPinned)));
    }
    if (filters.isFeatured !== undefined) {
      query.where(and(...baseConditions, eq(posts.isFeatured, filters.isFeatured)));
    }
    ...
    if (filters.createdAt) {
      if (filters.createdAt.start) {
        query.where(and(...baseConditions, gte(posts.createdAt, filters.createdAt.start)));
      }
      if (filters.createdAt.end) {
        query.where(and(...baseConditions, lte(posts.createdAt, filters.createdAt.end)));
      }
    }
    ...
    if (filters.authorId) {
      dynamicCountQuery.where(and(...baseConditions, eq(posts.authorId, filters.authorId)));
    }
    if (filters.postType) {
      dynamicCountQuery.where(and(...baseConditions, eq(posts.postType, filters.postType as any)));
    }
    if (filters.isPinned !== undefined) {
      dynamicCountQuery.where(and(...baseConditions, eq(posts.isPinned, filters.isPinned)));
    }
    if (filters.isFeatured !== undefined) {
      dynamicCountQuery.where(and(...baseConditions, eq(posts.isFeatured, filters.isFeatured)));
    }
    ...
    orderedQuery = orderedQuery.orderBy(
      ...
      ...sort.map(sortItem => {
        switch (sortItem.field) {
          case 'createdAt':
            return sortItem.order === 'asc' ? asc(posts.createdAt) : desc(posts.createdAt);
          case 'updatedAt':
            return sortItem.order === 'asc' ? asc(posts.updatedAt) : desc(posts.updatedAt);
          case 'title':
            return sortItem.order === 'asc' ? asc(posts.title) : desc(posts.title);
          case 'likeCount':
            return sortItem.order === 'asc' ? asc(posts.likeCount) : desc(posts.likeCount);
          case 'commentCount':
            return sortItem.order === 'asc' ? asc(posts.commentCount) : desc(posts.commentCount);
          default:
  ...
  static async getPosts(options: PostQueryOptions = {}) {
    ...
    if (filters.communityId) {
      query.where(and(...baseConditions, eq(posts.communityId, filters.communityId)));
    }
    if (filters.authorId) {
      query.where(and(...baseConditions, eq(posts.authorId, filters.authorId)));
    }
    if (filters.isPublished !== undefined) {
      query.where(and(...baseConditions, eq(posts.isPublished, filters.isPublished)));
    }
    ...
    for (const sortItem of sort) {
      switch (sortItem.field) {
        case 'createdAt':
          orderedQuery = orderedQuery.orderBy(sortItem.order === 'asc' ? asc(posts.createdAt) : desc(posts.createdAt));
          break;
        case 'updatedAt':
          orderedQuery = orderedQuery.orderBy(sortItem.order === 'asc' ? asc(posts.updatedAt) : desc(posts.updatedAt));
          break;
        default:
  ...
  static async getUserPostsInCommunity(userId: string, communityId: string, options: PostQueryOptions = {}) {
  ...
  static async getUserLikedPosts(userId: string, options: PostQueryOptions = {}) {
    const {
      page = 1,
      pageSize = 20,
      sort = [{ field: 'createdAt', order: 'desc' }]
